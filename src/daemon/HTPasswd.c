				   ...
				   CONST char *, encrypted)
{
    ...
    *result = (char)0;
    ...
    CTRACE(stderr, "Passwd...... Matching encrypted \"%s\" with \"%s\" => %s\n",
	   result, encrypted,
				     ...
				     CONST char *, filename)
{
    ...
    if (!fp) {
	CTRACE(stderr, "ERROR....... Unable to open password file \"%s\"\n",
    ...
    *pw = '\0';
	...
	if (2 == (status = HTAAFile_readPasswdRec(fp,user,pw))) {
	    CTRACE(stderr,
		   "Matching.... username \"%s\" agains pwrec \"%s:%s\"\n",
		   username, user, pw);
    ...
    CTRACE(stderr,"%s... password for %s\n",
	   ((status != EOF) ? "Correct.." : "INCORRECT"), username);
